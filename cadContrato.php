...
<html lang="pt-BR">
  ...
  <body class="w3-responsive">
	...
	<section class="w3-container">
        ...
        <form method="post" action="recContrato.php">
		<label>Código:</label>
		...
		<input type="date" name="dataInclusao">
		<br><br>
		<label>Serviço:</label>
		<select name="servicos">
			<?php
			include("conexao.php");
			$result = pg_query($connection, "SELECT * FROM tb_convenio_servico");
			while ($row = pg_fetch_assoc($result)) {
			  echo "<option value='".$row["codigo"]."'>".$row["servico"]." - ".$row["convenio"]."</option>";
			}
			?>
			</select>
		<br><br>
